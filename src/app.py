"""
Application Streamlit de démonstration pour la prédiction du trafic à Paris.
Ce script charge les modèles déjà entraînés (Baseline, HGB, LSTM, GRU),
récupère l'historique récent de trafic et la météo en ligne, puis affiche
une prédiction interactive dans l'interface Streamlit.
"""
...
def load_data(path: str) -> pd.DataFrame | None:
    """Charge le dataset local principal si le fichier existe, puis convertit la colonne horaire en datetime UTC."""
    path = Path(path)
    if not path.exists():
        return None
    df = pd.read_csv(path)
...
def load_lstm_bundle():
    """Charge le modèle LSTM déjà entraîné, ses métadonnées et ses normaliseurs pour l'inférence."""
...
@st.cache_resource
def load_gru_bundle():
    """Charge le modèle GRU déjà entraîné, ses métadonnées et ses normaliseurs pour l'inférence."""
...
@st.cache_resource
def load_hgb_bundle():
    """Charge le modèle HGB ainsi que son fichier de métadonnées."""
...
def get_baseline_predictor(df: pd.DataFrame):
    """Construit une baseline historique en calculant la moyenne du trafic par site, heure de la journée et jour de la semaine."""
...
def predict_baseline(agg: pd.DataFrame, id_site: str, target_dt_utc: pd.Timestamp) -> float:
    """Retourne la prédiction de la baseline pour un site et une date cible à partir des moyennes historiques."""
...
def ensure_columns(df: pd.DataFrame, columns: list[str]) -> pd.DataFrame:
    """Ajoute les colonnes manquantes avec des valeurs vides afin de garantir la compatibilité avec les modèles."""
    df = df.copy()
    for col in columns:
...
def predict_lstm_next_hour(state: pd.DataFrame, latest_obs: pd.Timestamp, bundle: dict):
    """Produit une prévision à une heure d'horizon avec le modèle LSTM à partir de la dernière séquence disponible."""
...
def predict_hgb_recursive(state: pd.DataFrame, latest_obs: pd.Timestamp, target_dt_utc: pd.Timestamp, bundle: dict):
    """Réalise une prédiction récursive multi-horizon avec HGB en réinjectant chaque prédiction dans l'historique."""
    meta = bundle["meta"]
    model = bundle["model"]
...
def predict_gru_recursive(state: pd.DataFrame, latest_obs: pd.Timestamp, target_dt_utc: pd.Timestamp, bundle: dict):
    """Réalise une prédiction récursive multi-horizon avec GRU en avançant heure par heure."""
    feats = bundle["feats"]
...
def main():
    ...
    """Construit l'interface Streamlit, charge les modèles disponibles et affiche la prédiction sélectionnée par l'utilisateur."""
    ...
    # Chargement optionnel d'un dataset local pour afficher les sites disponibles.
    ...
    # Détection des modèles réellement disponibles dans le dossier data/.
    ...
    # Appel à l'API Paris Open Data pour récupérer les observations récentes.
    ...
    # Construction de l'état courant utilisé pour la prédiction.
